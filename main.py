...
async def ensure_user(conn, discord_id: int):
    await conn.execute("""
        INSERT INTO users (discord_id, points, messages)
        VALUES ($1, 0, 0)
...
async def on_message(message: discord.Message):
    ...
    async with pool.acquire() as conn:
        await ensure_user(conn, message.author.id)
        # increment messages
        rec = await conn.fetchrow("SELECT messages, points FROM users WHERE discord_id=$1", message.author.id)
        messages = rec["messages"] + 1
        points = rec["points"]
        await conn.execute("UPDATE users SET messages=$1 WHERE discord_id=$2", messages, message.author.id)

        # award 1 Halloween point every 50 messages
        if messages % 50 == 0:
            points += 1
            await conn.execute("UPDATE users SET points=$1 WHERE discord_id=$2", points, message.author.id)
            embed = discord.Embed(
...
async def mystatus(interaction: discord.Interaction):
    async with pool.acquire() as conn:
        ...
        rec = await conn.fetchrow("SELECT points, messages FROM users WHERE discord_id=$1", interaction.user.id)
        embed = discord.Embed(
            title=f"Status for {interaction.user.display_name}",
            color=EMBED_COLOR,
            timestamp=datetime.utcnow()
        )
        embed.add_field(name="Halloween Points", value=str(rec["points"]), inline=True)
        embed.add_field(name="Messages tracked", value=str(rec["messages"]), inline=True)
        await interaction.response.send_message(embed=embed, ephemeral=True)

if __name__ == "__main__":
    if not DISCORD_BOT_TOKEN or not DATABASE_URL:
        print("Missing DISCORD_BOT_TOKEN or DATABASE_URL in environment.")
        exit(1)
