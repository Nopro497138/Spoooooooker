...
export default function Planko() {
  ...
  useEffect(() => {
    let mounted = true;
    fetch('/api/user', { cache: 'no-store' }).then(r => r.json()).then(j => { if (!mounted) return; setUser(j && j.discord_id ? j : null); }).catch(() => setUser(null));
    ...
    function drawStatic() {
      ...
      for (let r = 0; r < rows; r++) {
        for (let col = 0; col < cols; col++) {
          ...
          const y = 30 + r * 28;
          ctx.beginPath(); ctx.fillStyle = 'rgba(255,255,255,0.06)'; ctx.arc(x, y, 4, 0, Math.PI * 2); ctx.fill();
        }
      }
      // bottom multipliers
  ...
  async function play(e) {
    e.preventDefault();
    if (!user) { setResult({ error: 'Sign in first' }); setModalOpen(true); return; }
    const bet = Number(betRef.current.value || 0);
    if (!bet || bet <= 0) { setResult({ error: 'Enter a valid bet' }); setModalOpen(true); return; }
    if (bet > (user.candy || 0)) { setResult({ error: 'Insufficient candy' }); setModalOpen(true); return; }
    ...
    try {
      ...
      if (j.error) { setResult({ error: j.error }); setModalOpen(true); setRunning(false); return; }

      // animate ball falling to column j.column with gravity-like motion
      const c = canvasRef.current; const ctx = c.getContext('2d');
      const cols = 15;
      const startX = c.width / 2;
      const endX = 60 + j.column * ((c.width - 120) / (cols - 1));
      let x = startX, y = 20, vx = (endX - startX) / 60, vy = 0, g = 0.9;
      for (let t = 0; t < 80; t++) {
        ctx.clearRect(0, 0, c.width, c.height);
        // redraw pegs faintly
        for (let r = 0; r < 11; r++) {
      ...
      setResult(j);
      setModalOpen(true);
      const ures = await fetch('/api/user', { cache: 'no-store' }); const uj = await ures.json(); setUser(uj && uj.discord_id ? uj : null);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="container" style={{ paddingTop: 20 }}>
        <div className="card">
          <h2>Planko</h2>
          <p className="small">Drop the ball and hope it lands on a multiplier. Bets are validated server-side.</p>
        ...
        <aside className="card">
          <h4>Leaderboard</h4>
          <div className="small">Top players are shown on Home.</div>
        </aside>
      </div>
    </>
  );
